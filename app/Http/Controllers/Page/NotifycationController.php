...
class NotifycationController extends Controller
{
    ...
    public function notificationByCategory($category_id, Request $request)
    {
        ...
        $query = Notification::with(['user', 'category'])->where('category_id', $category_id);
        ...
        return view('pages.notify-category')->with('notification_categories', $notification_categories)
        ->with('current_category', $current_category)
            ->with('notifications', $notifications)->with('notification_latests', $notification_latests)
            ->with('current_filters', $request->all());
    }
    ...
    public function delete(Request $request){
        ...
        if ($notify->user_id != Auth::id()) {
            return redirect()->back()->with('error', 'Bạn không có quyền xóa thông báo này.');
        }
